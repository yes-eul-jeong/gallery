...
- [x] 저장소 뼈대, npm workspaces, Docker (node:22 + ffmpeg)
- [x] ffmpeg 인코딩 스크립트 (해상도 자동 판정, HLS 분할, 썸네일)
- [x] Cloudflare 계정, R2 버킷 생성, 업로드 스크립트
- [x] Worker: 서명 발급, 토큰 검증, Referer 검사, m3u8 재작성
- [x] 최소 HTML 페이지에서 hls.js 로 재생
...
테스트 영상(HEVC 1080p 22초, 8.8MB)으로 전 구간을 통과시켰다.

| 항목 | 결과 |
|---|---|
| 인코딩 | HEVC → H.264 변환, 12.7초 소요, 조각 4개 |
| 업로드 | R2 에 8개 파일 9.5MB |
| Referer 없이 서명 요청 | 403 |
| 정상 서명 요청 | 200, 30분 만료 토큰 발급 |
| 서명된 재생목록 | 200, 조각 주소에 토큰 주입 확인 |
| 토큰 없이 조각 직접 접근 | 403 |
| 위조 토큰 | 403 |
| 만료된 토큰 | 403 |
| 다른 사이트에서 삽입 | 403 |
| Range 요청 | 206, 요청한 바이트 수만큼 |
| 썸네일 (서명 면제) | 200 |
| 썸네일 Referer 없이 | 403 |
| CORS 헤더 | Access-Control-Allow-Origin 정상 |
| 스트림 전체 디코딩 | 성공 |
| init + 임의 조각 결합 | 성공 (브라우저 구간 이동 방식) |
...
첫 인코딩에서 결과물이 원본보다 커졌다(8.8MB → 12MB). 원본이 HEVC 3.05Mbps 로 이미 효율적으로
압축돼 있는데 H.264 CRF 22 가 4.3Mbps 를 쓴 탓이다.

재인코딩으로 화질이 원본보다 좋아질 수는 없으므로 비트레이트를 원본 이상 쓸 이유가 없다.
원본 비트레이트의 1.1배를 상한으로 두도록 고쳤고, 결과가 9.5MB 로 줄었다.
해상도도 같은 이유로 원본을 넘겨 확대하지 않는다. 구현은 `pipeline/src/config.ts` 의
`resolveEncodeSettings` 에 있다.

### 2단계 — 첫 화면과 경기 목록

- [x] Astro 프로젝트, 콘텐츠 스키마 정의
- [x] 샘플 경기 데이터 작성
- [x] 전적 집계 (`lib/record.ts`)
